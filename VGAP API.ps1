...
{
   $GameScores = (Invoke-PlanetsAPI "/game/loadscores?gameid=$gameID").scores|FT
   Return $GameScores
   ###
...
Function Get-PlanetsUserTurn($GameID,$UserName,$Password,$TurnNumber,$GamePlayerID)
{
   ...
   $URI = "/game/loadturn?GameID=$GameID&APIKey=$APIKey"
   If ($GamePlayerID -eq $null)
   {
      $GamePlayerID = ($GamePlayerInfo| ? {$_.username -eq $Username}).id
      If ($GameplayerID -eq $null) {Return 'This player is not part of this game. Please use the GamePlayerID value'}
...
{
   $status = Get-PlanetsGameStatus -GameID $Game ID
   If ($status -ne 'Finished')
   {
      'Game not complete, Message History cannot be conmplied'
      Return
   }
   $Players = (Get-PlanetsUserTurn -GameID $GameID -UserName $UserName -Password $Password -GamePlayerID 1 -turn 1).rst.players
   ...
   Foreach ($PlayerID in $playerIDs) 
   {
      ...
      $Loadturn = Get-PlanetsUserTurn -GameID $GameID -UserName $UserName -Password $Password -GamePlayerID $PlayerID
   ...
   {
      $SendingPlayerName = ($Players| ? {$_.id -eq $Message.ownerID}).Username
      $TargetPlayerName = ($Players| ? {$_.id -eq $Message.Target}).Username
...
Function Get-PLanetsHomeworlds ($GameID,$UserName,$Password)
{
   ...
   ### 
   $planets = (Get-PlanetsPlanetInfo -GameID $GameID -UseRName $UserName -Password $Password) | Select ID,X,Y
   ...
   Foreach ($Sourceplanet in $Planets)
   {
      ...
      $Planets0to81 = $PlanetstoReview | ? {($_.SourceplanetID -eq $SourcePlanetID) -and ($_.Distance -gt 0) -and ($_.Distance -le 81)} |Sort Distance
      $Planets81to162 = $PlanetstoReview | ? {($_.SourceplanetID -eq $SourcePlanetID) -and ($_.Distance -gt 81) -and ($_.Distance -lt 162)} |Sort Distance
      $planets155to162 = $PlanetstoReview | ? {($_.SourceplanetID -eq $SourcePlanetID) -and ($_.Distance -gt $OtherPlanetMinimumDistance) -and ($_.Distance -le 162)} |Sort Distance
...
Function Compare-PlanetsSettings ($GameID,$GameType,$BaselineGameID)
{
   ...
   Else
   {
      ...
      {
        $Options = [System.Management.Automation.Host.ChoiceDescription[]] @("&Standard","&Calssic","C&hamionship","C&ampaign","&Training","B&eginner")
        $Title = 'GameType'
...
Function Get-PlanetsBuildHistory ($GameID,$UserName,$Password,$GamePlayerID,$FilePath)
{
   ...
   $Planets = $GameData.rst.planets |Select Name,ID
   ...
   For ($TurnNumber=1;$TurnNumber -le $Lastturn;$TurnNumber++) 
   {
      ...
      $Starbases = $Turn.rst.starbases |? {$_.isbuilding -eq $True}
      If ($Starbases)
      {
         $Starbase = $null
         Foreach ($Starbase in $Starbases)
         {
            ...
            @{
               Turn = $TurnNumber
               Planet = ($Planets |? {$starbase.PlanetID -eq $_.id}).Name
               PLanetID = $Starbase.PlanetID
               StarbaseHull = ($Hulls |? { $Starbase.BuildhullID -eq  $_.id}).Name
               Engines = ($Engines |? { $Starbase.BuildEngineID -eq  $_.id}).Name
               Beams = ($Beams | ? { $Starbase.BuildBeamID -eq  $_.id}).Name
...
Function Get-PlanetsShipHistory ($GameID,$UserName,$Password,$GamePlayerID)
{
...
# Set variables
###
$Username= 'rfjohnson69@gmail.com'
$Password = 'Sprout69'
#$gameID = '342693'
$GameID = '329214'
#$GameID = 345852
...
Function Clone-PlanetsGame($UserName,$Paswword,$GameID)
{
   #############################################################################################################
   ###
   # Clone Game
   ...
   $BaseGameInfo = Get-PlanetsGameInfo -GameID $GameID
   $BaseGameSettings = $BaseGameInfo.Settings
   $BaseGamePlayers = $BaseGameInfo.Players


   ###
   # Set Settings
   # http://help.planets.nu/API-createcustomgame
   ###
   $NewGameName = Read-Host "Enter New Game Name (Default is the old Game Name -Clone)"
   If ($NewGameName -eq $null)
   {
      $oldGameName = $BaseGameInfo.name
      $NewGameName = "$OldGameName-Clone"
   }
...
"@ -split "`n"|%{$_.trim()}
$NewGameURI = $null
Foreach ($NewGameSettingName in $NewGameSettingNames)
{
   $NewGameSettingValue = ($GameSettings | ? {$_.name -eq $NewGameSettingName}).value
   if ($NewGameSettingName -eq 'name')
   {
      $NewGameSettingValue = Read-Host "Enter Game Name (default is `'old game name-clone`'"
      If ($NewGameSettingValue
      $NewGameSettingValue = "$NewGameSettingValue-Clone"}
   if ($NewGameSettingName -eq 'Description') {$NewGameSettingValue = $NewGamesettingValue -replace ' ', '+'}
   If (($NewGameSettingName -ne 'Players') -and ($NewGameSettingValue -ne $null))
   {
      $NewGameURI += "`"$NewGameSettingName`"`:`"$NewGameSettingValue`"`'"
   }
}
...
###
# Format Players Settings
# See above
###
$NewGamePlayerInfo =$Gameinfo.players|Select ID,RaceID
$NewGamePlayerSetting = '['
Foreach ($NewGamePlayer in $GamePlayerInfo)
{
   $NewGamePLayerID = $NewGamePlayer.ID
   $NewGamePlayerRaceID = $NewGamePlayer.RaceId
   $NewGamePlayerSetting = $NewGamePlayerSetting & {`"id`":$NewGamePlayerID`'`"raceid`":$NewGamePlayerRaceID}`'"
}
$NewGamePlayerSetting
   ...
   FORMAT- [{"id":1'"raceid":0}'{"id":2'"raceid":0}'{"id":3'"raceid":0}'{"id":4'"raceid":0}'{"id":5'"raceid":0}'{"id":6'"raceid":0}]



$NewGameFullURI = "/account/createcustomgame?apikey=$APIKey&Settings={$NewGameURI}"


Invoke-PlanetsAPI $NewGameFullURI